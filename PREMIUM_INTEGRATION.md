...
Create a separate plugin (e.g., `postpilot-pro`) that uses WordPress filters to override limits:
...
add_filter('postpilot_rate_limit_post', function($limit) {
    return 10; // Premium: 10 generations per post
});

// Increase per-post window to 10 minutes
add_filter('postpilot_rate_limit_post_window', function($window) {
    return 600; // Premium: 10 minutes
});

// Increase daily limit to 200
add_filter('postpilot_rate_limit_daily', function($limit) {
    return 200; // Premium: 200 per day
});

// Keep daily window at 24 hours
add_filter('postpilot_rate_limit_daily_window', function($window) {
...
function postpilot_pro_apply_limits() {
    // Check if user has valid license
    $license_key = get_option('postpilot_pro_license_key');
    
    if (postpilot_pro_verify_license($license_key)) {
        // Apply premium limits
        add_filter('postpilot_rate_limit_post', function() {
            return 10; // Premium limit
        });
        
        add_filter('postpilot_rate_limit_daily', function() {
            return 200; // Premium limit
        });
    }
}
add_action('plugins_loaded', 'postpilot_pro_apply_limits');

function postpilot_pro_verify_license($license_key) {
...
```php
<?php
function postpilot_apply_plan_limits() {
    $plan = get_option('postpilot_plan', 'free'); // free, starter, pro, enterprise
    
    switch ($plan) {
        case 'starter':
            add_filter('postpilot_rate_limit_post', fn() => 5);
            add_filter('postpilot_rate_limit_daily', fn() => 100);
            break;
            
        case 'pro':
            add_filter('postpilot_rate_limit_post', fn() => 10);
            add_filter('postpilot_rate_limit_daily', fn() => 200);
            break;
            
        case 'enterprise':
            add_filter('postpilot_rate_limit_post', fn() => 50);
            add_filter('postpilot_rate_limit_daily', fn() => 1000);
            break;
...
add_action('plugins_loaded', 'postpilot_apply_plan_limits');
```
...
|------------|---------------|-------------|
| `postpilot_rate_limit_post` | `2` | Generations per post |
| `postpilot_rate_limit_post_window` | `300` | Time window in seconds (5 min) |
| `postpilot_rate_limit_daily` | `30` | Generations per day |
| `postpilot_rate_limit_daily_window` | `86400` | Daily window in seconds (24 hours) |
...
/**
 * Plugin Name: PostPilot Pro
 * Plugin URI: https://yoursite.com/postpilot-pro
...
class PostPilot_Pro {
    ...
    public function __construct() {
        $this->license_key = get_option('postpilot_pro_license');
    ...
    public function render_license_page() {
        ...
        <div class="wrap">
            ...
            <form method="post" action="options.php">
                <?php
                settings_fields('postpilot_pro');
                do_settings_sections('postpilot_pro');
                ?>
...
add_filter('postpilot_rate_limit_post', fn() => 999);
add_filter('postpilot_rate_limit_daily', fn() => 9999);
```
